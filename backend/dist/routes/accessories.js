...
router.get('/api/accessories/:id', async (req, res) => {
    try {
        const found = await DatabaseRouter.findByIdAcrossDatabases(req.params.id, async (connection, id) => getAccessoryModel(connection).findById(id).lean(), 'accessory');
        if (!found) {
...
router.post('/api/accessories', requireAdmin, async (req, res) => {
    try {
...
router.patch('/api/accessories/:id', requireAdmin, async (req, res) => {
    try {
        const data = accessoryInputSchema.partial().parse(req.body);
        const found = await DatabaseRouter.findByIdAcrossDatabases(req.params.id, async (connection, id) => getAccessoryModel(connection).findById(id), 'accessory');
        if (!found) {
            res.status(404).json({ error: 'الإكسسوار غير موجود' });
            return;
        }
        ...
        const accessory = await DatabaseRouter.updateOnDatabase(found.dbIndex, async (connection) => getAccessoryModel(connection).findByIdAndUpdate(req.params.id, patch, { new: true }), 'accessory');
...
router.delete('/api/accessories/:id', requireAdmin, async (req, res) => {
    try {
        const found = await DatabaseRouter.findByIdAcrossDatabases(req.params.id, async (connection, id) => getAccessoryModel(connection).findById(id), 'accessory');
        if (!found) {
            res.status(404).json({ error: 'الإكسسوار غير موجود' });
        ...
        await DatabaseRouter.deleteFromDatabase(found.dbIndex, async (connection) => getAccessoryModel(connection).findByIdAndDelete(req.params.id), 'accessory');
