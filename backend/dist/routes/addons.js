...
router.get('/api/addons', async (req, res) => {
    try {
        ...
        if (compatibleWith && typeof compatibleWith === 'string') {
            items = items.filter(a => a.compatibleWith.length === 0 || a.compatibleWith.includes(compatibleWith));
        }
...
router.post('/api/addons', requireAdmin, async (req, res) => {
    try {
...
router.patch('/api/addons/:id', requireAdmin, async (req, res) => {
    try {
        const data = addonInputSchema.partial().parse(req.body);
        const found = await DatabaseRouter.findByIdAcrossDatabases(req.params.id, async (connection, id) => getAddonModel(connection).findById(id), 'addon');
        if (!found) {
        ...
        const addon = await DatabaseRouter.updateOnDatabase(found.dbIndex, async (connection) => getAddonModel(connection).findByIdAndUpdate(req.params.id, patch, { new: true }), 'addon');
...
router.delete('/api/addons/:id', requireAdmin, async (req, res) => {
    try {
        const found = await DatabaseRouter.findByIdAcrossDatabases(req.params.id, async (connection, id) => getAddonModel(connection).findById(id), 'addon');
        if (!found) {
            res.status(404).json({ error: 'الإضافة غير موجودة' });
        ...
        await DatabaseRouter.deleteFromDatabase(found.dbIndex, async (connection) => getAddonModel(connection).findByIdAndDelete(req.params.id), 'addon');
