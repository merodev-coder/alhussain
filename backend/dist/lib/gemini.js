...
export async function normalizePricelistWithGemini(rawRows) {
    ...
    logInfo('Gemini Normalization', `Starting sequential normalization for ${rawRows.length} rows in batches of ${BATCH_SIZE}`);
    for (let i = 0; i < rawRows.length; i += BATCH_SIZE) {
        const batch = rawRows.slice(i, i + BATCH_SIZE);
        const startIndex = i + 1;
        const batchNumber = Math.floor(i / BATCH_SIZE) + 1;
        const totalBatches = Math.ceil(rawRows.length / BATCH_SIZE);
        logInfo('Gemini Normalization', `Processing batch ${batchNumber}/${totalBatches} (${batch.length} rows) sequentially`);
        try {
...
`;
    ...
    try {
        ...
        logError(`Primary model (${primaryModel}) failed. Error: ${primaryError?.message || primaryError}. Attempting fallback to ${fallbackModel}`, primaryError);
