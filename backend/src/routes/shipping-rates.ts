...
router.get('/api/shipping-rates', async (_req: Request, res: Response): Promise<void> => {
  try {
    const all = await DatabaseRouter.readAcrossAllDatabases(
      async connection => getShippingRateModel(connection).find().sort({ governorate: 1 }).lean(),
      'shippingRates'
    )
    const byGov = new Map<string, (typeof all)[number]>()
    for (const row of all) {
      if (!byGov.has(row.governorate)) byGov.set(row.governorate, row)
    }
...
router.patch(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const all = await DatabaseRouter.readAcrossAllDatabases(
        async connection => getShippingRateModel(connection).find({ governorate }).lean(),
        'shippingRate'
      )
      ...
      const updated = await DatabaseRouter.updateOnDatabase(
        dbIndex,
        async connection =>
          getShippingRateModel(connection).findOneAndUpdate({ governorate }, data, { new: true }),
        'shippingRate'
      )
