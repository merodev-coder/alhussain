...
const router = Router();
router.get('/api/inventory', requireAdmin, async (req, res) => {
    try {
        const { category, lowStock } = req.query;
        const [products, addons, accessories] = await Promise.all([
            DatabaseRouter.readAcrossAllDatabases(async (connection) => getProductModel(connection).find().lean(), 'products'),
            DatabaseRouter.readAcrossAllDatabases(async (connection) => getAddonModel(connection).find().lean(), 'addons'),
            DatabaseRouter.readAcrossAllDatabases(async (connection) => getAccessoryModel(connection).find().lean(), 'accessories'),
        ]);
        let rows = [
...
router.get('/api/inventory/logs', requireAdmin, async (_req, res) => {
    try {
        const logs = await DatabaseRouter.readAcrossAllDatabases(async (connection) => getInventoryLogModel(connection).find().sort({ date: -1 }).limit(200).lean(), 'inventoryLogs');
...
router.get('/api/inventory/export.csv', requireAdmin, async (_req, res) => {
    try {
        const [products, addons, accessories] = await Promise.all([
            DatabaseRouter.readAcrossAllDatabases(async (connection) => getProductModel(connection).find().lean(), 'products'),
            DatabaseRouter.readAcrossAllDatabases(async (connection) => getAddonModel(connection).find().lean(), 'addons'),
...
router.patch('/api/inventory', requireAdmin, async (req, res) => {
    try {
        ...
        const lookup = data.itemType === 'laptop'
        ...
        if (data.itemType === 'laptop') {
            await DatabaseRouter.updateOnDatabase(lookup.dbIndex, async (connection) => getProductModel(connection).findByIdAndUpdate(data.itemId, {
                quantity: data.quantity,
                stockStatus,
            }), 'product');
        }
        else if (data.itemType === 'addon') {
            await DatabaseRouter.updateOnDatabase(lookup.dbIndex, async (connection) => getAddonModel(connection).findByIdAndUpdate(data.itemId, {
                quantity: data.quantity,
                stockStatus,
            }), 'addon');
        }
        else {
            await DatabaseRouter.updateOnDatabase(lookup.dbIndex, async (connection) => getAccessoryModel(connection).findByIdAndUpdate(data.itemId, {
                quantity: data.quantity,
                stockStatus,
            }), 'accessory');
        }
