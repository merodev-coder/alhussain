...
router.post('/api/orders', orderRateLimit, async (req, res) => {
    try {
        ...
        for (const item of data.items) {
            ...
            const catalog = itemType === 'accessory'
            ...
            for (const addonSel of item.selectedAddons) {
                ...
                const compatible = addon.compatibleWith.length === 0 || addon.compatibleWith.includes(item.productId);
                if (!compatible) {
...
router.get('/api/orders/:id', requireAdmin, async (req, res) => {
    try {
        const found = await DatabaseRouter.findByIdAcrossDatabases(req.params.id, async (connection, id) => getOrderModel(connection).findById(id).lean(), 'order');
        if (!found) {
...
router.patch('/api/orders/:id', requireAdmin, async (req, res) => {
    try {
        ...
        const found = await DatabaseRouter.findByIdAcrossDatabases(req.params.id, async (connection, id) => getOrderModel(connection).findById(id), 'order');
        if (!found) {
            res.status(404).json({ error: 'الطلب غير موجود' });
            return;
        }
        const current = found.result;
        const shouldDecrement = statusValidation.data === 'shipped' && !current.stockDecremented && current.status !== 'shipped';
        const shouldSendReceipt = statusValidation.data === 'completed' && current.status !== 'completed';
        if (shouldDecrement) {
            await decrementStockForOrder(current.toObject());
        }
        const order = await DatabaseRouter.updateOnDatabase(found.dbIndex, async (connection) => getOrderModel(connection).findByIdAndUpdate(req.params.id, {
...
router.patch('/api/orders/:id/payment', requireAdmin, async (req, res) => {
    try {
