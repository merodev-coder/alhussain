...
async function decrementById(kind, id, amount) {
    if (amount <= 0)
        return;
    if (kind === 'laptop') {
        ...
        await DatabaseRouter.updateOnDatabase(item.dbIndex, async (connection) => getProductModel(connection).findByIdAndUpdate(id, {
            quantity: nextQty,
    ...
    if (kind === 'addon') {
        ...
        const nextQty = Math.max(0, (item.quantity ?? 0) - amount);
        await DatabaseRouter.updateOnDatabase(item.dbIndex, async (connection) => getAddonModel(connection).findByIdAndUpdate(id, {
            quantity: nextQty,
            stockStatus: suggestStockStatus(nextQty),
        }), 'addon');
    ...
    const nextQty = Math.max(0, (item.quantity ?? 0) - amount);
    await DatabaseRouter.updateOnDatabase(item.dbIndex, async (connection) => getAccessoryModel(connection).findByIdAndUpdate(id, {
        quantity: nextQty,
        stockStatus: suggestStockStatus(nextQty),
    }), 'accessory');
...
export async function decrementStockForOrder(order) {
    for (const line of order.items) {
        try {
            ...
            logError('Inventory', `Failed decrementing line ${line.productId} on order ${order.orderNumber}: ${error}`);
        }
    }
}
