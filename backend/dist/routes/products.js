...
router.get('/api/products/:id', async (req, res) => {
    try {
        ...
        const result = await DatabaseRouter.findByIdAcrossDatabases(id, async (connection, productId) => getProductModel(connection).findById(productId).lean(), 'product');
        if (!result) {
...
router.patch('/api/products/:id', requireAdmin, async (req, res) => {
    try {
        ...
        const product = await DatabaseRouter.updateOnDatabase(found.dbIndex, async (connection) => getProductModel(connection).findByIdAndUpdate(req.params.id, patch, { new: true }), 'product');
...
router.delete('/api/products/:id', requireAdmin, async (req, res) => {
    try {
        ...
        await DatabaseRouter.deleteFromDatabase(found.dbIndex, async (connection) => getProductModel(connection).findByIdAndDelete(req.params.id), 'product');
